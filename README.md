...
- **テーブル**: ツールバーから挿入、右クリックで行/列の追加・削除
  ...
  - **セル範囲選択**: マウスドラッグで連続するセルを矩形選択（選択範囲は1つの矩形）
  - **セル範囲コピー/貼り付け**: 選択セルをTSV形式でコピーし、TSV/HTMLテーブルを対応するセルへ貼り付け
  - **セル移動**: Tab/Shift+Tabで前後のセルへ移動、矢印キーで隣接セルへ移動
...
| `Tab` / `Shift+Tab` | 表セル内: 次/前のセルへ移動、行末では次/前の行へ移動 |
| `↑` `↓` `←` `→` | 表セル内: 上下左右のセルへ移動 |
| `Esc` | 表セルの編集モードから選択モードへ移行 |
| `Ctrl+;` | 日付挿入 |
| `Ctrl+:` | 時刻挿入 |
...
- **表セル範囲コピー**: マウスドラッグで選択した矩形範囲をTSV形式でコピー（`Cmd/Ctrl+C`）
- **表セル範囲貼り付け**: コピーしたTSV/HTMLテーブルを選択範囲の左上から対応するセルへ貼り付け（`Cmd/Ctrl+V`）
