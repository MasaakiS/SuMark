...
#### 準備フェーズ
0. **⚠️ 全差分を必ず確認（push漏れ防止）**
   ```bash
   git status          # 変更ファイルを全て把握
   git diff --stat HEAD  # 差分サマリーを確認
   ```
   未コミットの変更がある場合は、今回のリリースに含めるかどうかを判断してから進む。  
   `git add -A` または個別 `git add` でステージングし、漏れがないことを確認する。
