...
## [v1.0.6] - 2026-05-28
### 修正
- bare URL（`https://...`、`www.xxx`）を含むファイルを開いて保存すると `[url](url)` 形式に変化してしまう問題を修正
  - Turndown カスタムルール `bareUrlLink` を追加し、GFM autolink が生成した `<a href="url">url</a>`（href とテキストが同一）を bare URL 形式のまま保存するよう変更
  - 明示リンク `[text](url)`（テキスト ≠ URL）は従来どおり保存される

### 改善
- モーダルダイアログ（未保存確認・各種入力ダイアログ）表示時に OK ボタンへ自動フォーカスし、閉じると元のフォーカス先に戻るよう改善
