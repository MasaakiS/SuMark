...
## [v1.0.8] - 2026-05-30
### 修正
- 自動変換ルールの説明コメントを実装意図に合わせて更新し、タスクリスト短縮記法（`[]` / `[ ]`）の扱いを明記（`src/modules/autoConvert.js`）

### ドキュメント
- 依存関係ビジュアル図を最新構成へ更新（`docs/DEPENDENCY_MAP_VISUAL.md`）
- モジュール構成ドキュメントを最新化（`docs/MODULES.md`）
- README の関連説明を更新（`README.md`）
