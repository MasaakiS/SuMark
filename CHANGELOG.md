...
## [v1.2.1] - 2026.09.16
### 修正
- Windows環境でコードブロック内の文字列を選択してEnterを押した際、選択範囲を削除せず改行を追加していた問題を修正
- コードブロック内のEnter処理で選択範囲を改行に置換し、キャレット位置を復元するよう改善

### テスト
- `test/playwright/02-markdown.spec.js`に、コードブロック内で選択範囲をEnterで改行に置換する回帰テストを追加
- Markdown変換テスト29件が全件通過

## [v1.2.0] - 2026.09.05
### 変更
- 検索・置換で書式境界やコードハイライトのトークン境界をまたぐ文字列に対応
- 単語単位検索オプションと選択範囲内検索オプションを削除
- 正規表現モードで円記号（U+00A5）をバックスラッシュとして扱うよう改善

### テスト
- `test/playwright/14-find-dialog.spec.js`
- 既存 E2E テスト264件が全件通過

## [v1.1.4] - 2026.09.01
### 修正
- 検索ダイアログの単語単位検索オプションを削除
- 検索ダイアログの選択範囲内検索オプションを削除
- テーブルセルへ複数セル分のTSVまたはHTMLテーブルを貼り付けた後、貼り付け先の範囲を既存の矩形選択として選択状態にするよう修正
- 貼り付け後の矩形選択をCmd/Ctrl+Cで再コピーでき、既存の選択解除操作を維持
- 単一セルへの通常テキスト貼り付けでは矩形選択を開始せず、キャレット貼り付けを維持
- Windows WebView2でネイティブ選択Rangeを取得できない貼り付けでも、イベント対象セルから貼り付け先を判定して矩形選択を維持するよう修正
- テーブルの自動変換処理後に貼り付け先のセルを再取得して矩形選択を設定し、DOM更新後も選択表示が失われないよう修正

### テスト
- `test/playwright/04-table.spec.js`に、矩形選択コピー後の貼り付け先再選択・再コピー、および単一セル貼り付けでの非選択を検証する回帰テストを追加
- ネイティブ選択Rangeが空のWindows相当状態で、貼り付け先範囲の選択と再コピーを検証する回帰テストを追加
- `npm run test:e2e -- test/playwright/04-table.spec.js`
- テーブル操作テスト35件が全件通過

## [v1.1.3] - 2026.09.01
### 修正
- テーブルセル内のマウスドラッグで、同一セル内ではブラウザ標準のテキスト選択を維持し、別セルへドラッグした場合のみ矩形選択を開始するよう改善
- 複数セルをコピーしたTSV/HTMLテーブルを、貼り付け先の左上セルだけを指定して貼り付けられるよう改善
  - 貼り付け範囲が表の右端または下端を越える場合は、必要な列と行を自動追加
- 表外にキャレットがある状態でEnterを押しても、直近に編集した表セルへ誤って改行を挿入しないよう修正
- 表セル内の左右矢印キーによるセル移動を、キャレットがセル先頭または末尾にある場合だけに限定

### テスト
- `test/playwright/04-table.spec.js`に、単一セル内のテキスト選択、矩形選択の貼り付け範囲拡張、表外Enter、左右矢印キーの境界移動に関する回帰テストを追加
- `npm run test:e2e -- test/playwright/04-table.spec.js`
- テーブル操作テスト32件が全件通過

## [v1.1.2] - 2026.08.27
### リリース
- v1.1.1のリリース状態を引き継ぎ、バージョン番号をv1.1.2へ更新
