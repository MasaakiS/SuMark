...
## [v1.0.8] - 2026-05-29
### 追加
- テーブルで列を範囲選択し、選択セル全体に同じ内容を一括入力できる機能を追加（Excel 的操作感）
  - セルをクリックするとアンカーセル（青枠）として記録される
  - 同じ列の別セルを **Shift+クリック** するとアンカーから選択範囲をハイライト（薄い青）
  - アンカーセルで文字を入力後、**Ctrl+Enter**（Mac: Cmd+Enter）を押すと選択セル全体に同じ内容を一括入力
  - テーブル外クリックまたは **Escape** キーで選択解除
  - Ctrl+Z で元に戻せる（Undo 対応）
  - 実装箇所: `tableManager.js`（選択管理・イベント）、`keyboard.js`（Ctrl+Enter ハンドラ）、`markdown.css`（`.col-anchor` / `.col-selected` スタイル）

## [v1.0.7] - 2026-05-29
### 修正
- 未保存変更確認ダイアログ（タブ閉じ・アプリ終了）表示時に Enter キーで文書側が改行されてしまう問題を修正
  - ダイアログ表示直後に OK ボタンへ自動フォーカスし（`requestAnimationFrame`）、閉じると元の要素にフォーカスを戻すよう改善（`tabManager.js`、`main.js`）
- 検索ダイアログで一致テキストが選択状態になり、Enter キーで文書の一致テキストが削除されてしまう問題を修正
  - `moveToSearchHighlight()` から `sel.addRange()` によるドキュメント選択を除去し、CSS クラスによるハイライトのみに変更

### 改善
- 検索ダイアログをモードレスなフローティングダイアログに刷新（`toolbarActions.js`、`dialogs.css`）
  - オーバーレイなしで文書操作と検索を同時に行えるよう変更
  - ダイアログはドラッグ可能。「次へ」クリック後もフォーカスが検索入力欄に留まる
  - ヒット件数（N / M 形式）をリアルタイム表示。大/小文字区別のチェックボックスを装備
  - Escape キーでダイアログを閉じてエディタにフォーカスを戻す

### 改善（テーブル）
- テーブルの列選択機能を追加（`tableManager.js`、`keyboard.js`、`markdown.css`）
  - 同一列のセルを複数選択し、アンカーセルの内容を選択セル全体に一括反映（Cmd/Ctrl+Enter）
  - 選択中セルはハイライト表示（アンカー: 青、選択: 薄青）

### テスト
- 検索ダイアログ（モードレス）の E2E テストを新規追加（`14-find-dialog.spec.js`、21 テスト）
  - ダイアログの開閉、ハイライト動作、フォーカス挙動、選択テキスト自動入力などを網羅
  - 全 198 テスト通過確認済み
